#include "MC_NVT_poly_sticky_spheres_equil.h"
...
int main(int argc, char *argv[]){
	...
	strcat(sys_info_file_name, argv[6]);
	strcat(sys_info_file_name, ".txt");

	strcpy(restart_file_name,getenv("PWD"));
	strcat(restart_file_name,"/restart_files");
	mkdir(restart_file_name, S_IRWXU);
	strcat(restart_file_name,"/");
	strcat(restart_file_name, argv[0]);
	strcat(restart_file_name, "__restart__");
	strcat(restart_file_name, argv[6]);
	strcat(restart_file_name, ".xyz");
	...
	fprintf(sys_info_out, "#RNG_Seed %d #phi_i %f #phi_f %f #u %f #lambda %f #dr %f #Init_config %s #type_stamp %s\n", seed, phi_i, phi_f, u, lambda, dr, argv[5], argv[7]);
	...
	while(time_current < SIMULATION_TIME_MAX) {
		...
		integrator_1->run_dynamics_particle(time_step*double(NUMBER_PARTICLES));

		time_current = time_current + time_step;

		box_ti -> dump_configuration(strdup(file_names[next_step].c_str()), seed, argv[5], dr, dv, argv[7]);
		box_ti -> dump_restart(restart_file_name, jobid, argv[5], dr, dv, argv[7]);
