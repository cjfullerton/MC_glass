#include "integrator_MC_NVT.h"


double integrator_MC_NVT::run_dynamics(double moves) {
	...
	for(int l=0; l<(moves); ++l){
		...
		if(dE <= 0.0) { accept_count = accept_count +1.0;}
			...
			if(rand_1->prob_gen()<=Prob){accept_count = accept_count+1.0;}
			else {
	...
	box_1->c_timestamp(box_1->r_timestamp() + moves/box_1->r_number_particles());
