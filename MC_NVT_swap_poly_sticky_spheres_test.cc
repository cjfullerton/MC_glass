#include "MC_NVT_swap_poly_sticky_spheres_test.h"
...
int main(int argc, char *argv[]){
	...
	double L, phi_i, pressure_f, time_step, log_step, time_end, count, waiting_time, simulation_time, time_current, dv, dr, delta_size_cut, prob_swap_move, lambda, u;
	FILE *config_in;
	FILE *sys_info_out;
	char file_name[1024] = {0};
	char sys_info_file_name[1024] = {0};
	char acceptance_file_name[1024] = {0};
	vector <std::string> file_names;

	stringstream convert_phi_i(argv[2]); convert_phi_i >> phi_i;
	...
	stringstream convert_delta_size_cut(argv[7]); convert_delta_size_cut >> delta_size_cut;

	pressure_f = 0.0;

	prob_swap_move = 0.2;

	stringstream convert_jobid(argv[9]); convert_jobid >> jobid;
	...
	strcat(sys_info_file_name, argv[9]);
	strcat(sys_info_file_name, ".txt");

	strcpy(acceptance_file_name,getenv("PWD"));
	strcat(acceptance_file_name,"/sys_info_data");
	mkdir(acceptance_file_name, S_IRWXU);
	strcat(acceptance_file_name, "/");
	strcat(acceptance_file_name, argv[0]);
	strcat(acceptance_file_name, "__acceptance__");
	strcat(acceptance_file_name, argv[9]);
	strcat(acceptance_file_name, ".txt");
	...
	box_ti->c_packing_fraction(phi_i);

	fclose(config_in);

	integrator_MC_NPT_swap *integrator_1 = new integrator_MC_NPT_swap(box_ti, dr, dv, delta_size_cut, prob_swap_move, rand_1);

	time_current = 0.0;

	sys_info_out = fopen(sys_info_file_name, "a");
	fprintf(sys_info_out, "#RNG_Seed %d #phi_i %f #pressure_f %f #u %f #lambda %f #dr %f #dV %f #delta_cut %f #prob_swap %f #Init_config %s #type_stamp %s\n", seed, phi_i, pressure_f, u, lambda, dr, dv, delta_size_cut, prob_swap_move, argv[1], argv[8]);
	...
	while(time_current < SIMULATION_TIME_MAX) {
		...
		integrator_1->run_dynamics_particle_swap(time_step*double(NUMBER_PARTICLES));

		time_current = time_current + time_step;

		box_ti -> dump_configuration(strdup(file_names[next_step].c_str()), seed, argv[1], dr, dv, argv[8]);
		...
		integrator_1->print_moves(acceptance_file_name, box_ti->r_timestamp());
	}
